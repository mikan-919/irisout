...
// を書き出すビルドスクリプト(plans/001-browser-build-target.md)。
// CONCEPT.v2.md「ブラウザに届くのは、静的 HTML と専用の更新コードだけで
// あるべき」を検証する: 初期 HTML は index.html に焼き込み、app.js は
// それを hydrate するだけ(innerHTML は書かない)。
...
if (!input) {
  console.error('usage: bun scripts/build.ts <entry.jsx>')
  process.exit(1)
}
...
// (src/codegen.ts:44)。test/helpers.ts と同じやり方で絶対パスに書き換える。
...
for (const file of [indexPath, path.join(outdir, 'app.js')]) {
  console.log(
    `${path.relative(process.cwd(), file)} (${statSync(file).size} bytes)`,
  )
}
