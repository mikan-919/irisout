...
const built = await Bun.build({
  entrypoints: ['bench/browser-driver.ts'],
  target: 'browser',
  format: 'esm',
  define: { 'process.env.NODE_ENV': '"production"' },
})
if (!built.success) {
  console.error('driver bundle failed:', built.logs)
  process.exit(1)
}
...
async function measureCell(impl: Impl, n: number) {
  ...
  const run = () =>
    page.evaluate(
      ([impl, n]) => window.__bench.runScenarios(impl as Impl, n as number),
      [impl, n] as const,
...
async function measureToggleCell(impl: Impl, n: number): Promise<number> {
  ...
  const run = () =>
    page.evaluate(
      ([impl, n]) => window.__bench.runToggleAll(impl as Impl, n as number),
      [impl, n] as const,
    ) as Promise<number>
  await run() // warm-up(捨てる)
  const runs: number[] = []
