...
const { mountComponent } = await import('../examples/todomvc.handwritten.js')
const React = await import('react')
const { createRoot } = await import('react-dom/client')
const { flushSync } = await import('react-dom')
const { TodoApp } = await import('../examples/todomvc.react.tsx')
...
function mountReact(todos: Todo[]) {
  ...
  const t0 = performance.now()
  flushSync(() =>
    root.render(React.createElement(TodoApp, { initialTodos: todos })),
  )
  const mountMs = performance.now() - t0
  const heapDeltaKb = heapUsedKb() - beforeKb
  return {
...
function switchFilters(handle: Handle): number {
  const active = handle.container.querySelector(
    '[data-filter="active"]',
  ) as HTMLElement | null
...
function addOne(handle: Handle): { ms: number; preservedExisting: boolean } {
  const existingLis = Array.from(
    handle.container.querySelectorAll('.todo-list li'),
  )
  ...
  const preservedExisting =
    nowLis.length === existingLis.length + 1 &&
    existingLis.every((li, i) => nowLis[i] === li)
...
for (const n of SIZES) {
  ...
  if (!hw.addOnePreservedExisting) {
    console.log(
      '  WARNING: handwritten did not preserve existing <li> identity on add',
    )
  }
