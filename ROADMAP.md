...
[yt-util移植計画](./docs/yt-util-migration-plan.md)に従い、irisout・Hono RPCを組み合わせる利用例を実際にドッグフーディングしながら検証する。ログイン導線、プレイリスト取得、視聴済み保存の経路を確認したため、次は視聴済み状態を使った並べ替えと一覧操作を移す。利用中に再現した不足は小さな変更として直し、必要になった接続処理をフルスタック対応の検討材料にする。irisout本体の機能追加は、次のいずれかを確認した場合に着手する。
