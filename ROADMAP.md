...
(`onClick={() => toggle(todo.id)}`)、`toggle` 本体の signal 書き込みは
解析されず `update_*()` が発火しない — **黙って更新が落ちる**(M2 以来の
挙動、change `dynamic-attribute-bindings` のテスト作成中に発見)。
識別子参照ハンドラ(`onClick={toggle}`)なら本体が解析されるので動く。
引数を渡したいアイテム内ハンドラで踏みやすい。

決めること(いずれか):
- (a) **拒否**: ハンドラ本体から動きゾーン関数への呼び出しを scope limit
  にする(「黙って落とすとバグの温床」の規約に沿う最小対応。誤検知の
  範囲 — 追跡と無関係なユーティリティ呼び出しまで拒否するか — が論点)。
- (b) **追跡**: 呼び出し先が動きゾーンの function 宣言なら、その本体の
  writeDeclIds をハンドラへ合流させる(1階層のみ・再帰なしなら実装は
  小さいが、コンパイラの解析範囲が関数境界を越える初の前例になる)。
- (c) **現状維持を明文化**: 「書き込みはハンドラ本体に直接書く」を
  authoring 規約として conventions/ADR に明記するだけ。

判断したら ADR 化する(却下案も含む)。それまで STATUS.md の既知の制約
には積まない(仕様か バグかの判断自体が本論点のため)。
