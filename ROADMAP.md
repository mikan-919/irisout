...
[yt-util移植計画](./docs/yt-util-migration-plan.md)に従い、irisout・Hono RPCを組み合わせる利用例を検証する。まずログイン、プレイリスト取得、視聴済み保存までを通し、必要になった接続処理をフルスタック対応の検討材料にする。irisout本体の機能追加は、次のいずれかを確認した場合に着手する。
