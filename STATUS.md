...
ソース版とnpm公開版は0.3.0である。0.3.0は記述APIの名前付きimport、ブラウザ用コンパイラ、要求ごとのサーバー描画、Honoファイル経路、Motionの退場処理を含み、2026-09-26にnpmへ公開した。梱包検査、文書生成検査、npm公開版を使う別アプリ導入検査を通過した。0.2.2は文書とアプリ開発用スキルを更新し、公開入口と生成コードは変更していない。0.2.1では状態更新を
