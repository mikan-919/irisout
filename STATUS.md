...
同一生成物・同一Chromium・同一反復数の基準を取り直し、List初期化は82.7msから77.8msへ
5.9%短縮した。TodoMVCの測定は環境差を含み、R3の20%短縮は未達成として扱う。
...
の直接イベント配線を維持した共有callbackを追加した。同一生成物・同一Chromium・同一反復数で
1万件のList初期化は82.7msから77.8msとなり、短縮率は5.9%だった。20%短縮は未達成で、探索
