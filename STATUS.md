...
1万件のList初期化は82.7msから69.9msとなり、短縮率は15.5%だった。20%短縮は未達成で、探索
方式を変える案は測定値が悪化したため採用しない。追加改善または目標改訂をADR-0044へ記録する。
...
14.5%短縮だったが、前回候補比較は4.6%であり、環境ばらつきを含め20%目標は未達である。
限定実装は採用するが、R3完了や目標達成は宣言しない。`e645893`は今回変更前の比較対象で、
