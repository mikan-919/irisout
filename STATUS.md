...
ソース版は0.3.0、npm公開版は0.2.2である。0.3.0は記述APIの名前付きimport、ブラウザ用コンパイラ、要求ごとのサーバー描画、Honoファイル経路、Motionの退場処理を含む公開準備版である。梱包検査と文書生成検査は通過した。npm公開は認証がなく未完了である。0.2.2は文書とアプリ開発用スキルを更新し、公開入口と生成コードは変更していない。0.2.1では状態更新を
