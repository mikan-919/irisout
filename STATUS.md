...
現在全件描画である。yt-util側へPlaywrightの固定データ比較を追加し、同じChromiumと100・1000・
10000件の入力で、初期表示、検索、選択、スクロールを測定した。10000件では全件描画の初期表示と
検索が遅くなり、irisout側の表示行数は全件、Next.js側の仮想一覧は10行だった。条件と数値は
yt-utilの`docs/playlist-performance.md`に記録した。
