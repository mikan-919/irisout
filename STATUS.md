...
2026-09-15にPlayground開発入口で、公式サイト用と実行管理用のVite+が同じ依存最適化
キャッシュを同時更新し、`ENOTEMPTY`で停止する問題を修正した。二つの開発サーバーへ別々の
`cacheDir`を渡し、並行起動と各画面の取得を確認した。単独の`dev:web`は従来の既定キャッシュを使う。
