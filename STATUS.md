...
ソース版とnpm公開版のlatestは0.3.3である。0.3.3は2026-09-26にnpmへ公開した。Hono開発時に、Honoのmiddlewareが生成HTMLをViteへ渡さず、ブラウザーへHMR clientが入らない問題を特定した。開発HTMLへViteのHMR clientを挿入し、更新されたpage moduleをViteのHMR境界で差し替えて現在のpageだけを再hydrateする。文書全体は再読込せず、page内signal状態はSSR初期値へ戻る。経路の追加・削除では開発サーバーを再起動する。package buildと公式site buildは通過したが、開発サーバー上のHMRは未確認である。0.3.0は記述APIの名前付きimport、ブラウザ用コンパイラ、要求ごとのサーバー描画、Honoファイル経路、Motionの退場処理を含む。梱包検査、文書生成検査、npm公開版を使う別アプリ導入検査を通過した。0.2.2は文書とアプリ開発用スキルを更新し、公開入口と生成コードは変更していない。0.2.1では状態更新を
...
2026-09-26にHono開発時のpage変更でSSR側の描画関数が起動時の内容を保持する不具合を修正した。
追加調査で、Honoの開発middlewareがViteのHTML変換を通さず、HMR clientが文書へ入らないことを特定した。
開発HTMLを`transformIndexHtml()`へ渡す。page変更時はclientとSSRを再コンパイルし、ViteのHMR境界で更新moduleを受け、現在のpageを再hydrateする。
page追加・削除では経路表が変わるため、開発サーバーを再起動して全体を再読込する。page内signal状態はSSR初期値へ戻る。
package buildと公式site buildは通過した。開発サーバーでのHMRは未確認である。
...
- `bun run registry:smoke`: npm公開版0.3.3に同じ隔離検査を適用。
