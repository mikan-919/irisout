...
// ラッパーが残らない、(b) runtime import が mount/hydrate のみ、(c) 実 DOM で
...
describe('M6: cross-milestone no-wrapper verification (ADR-0006)', () => {
  ...
  it('imports only mount/hydrate from the runtime, in a single import', () => {
    const { code } = compile(ALL_FEATURES_SOURCE)
    const imports = code.match(/^import .*$/gm) ?? []
    expect(imports).toEqual([
      "import { mount, hydrate } from '../src/runtime.js';",
    ])
  })
