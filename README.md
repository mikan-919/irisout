...
> [!WARNING]
> npmで公開中の`irisout`は0.3.3です。0.3.4でHono構成のHMR修正を準備しています。試用版のため、対応範囲を確認してください。
...
| `bun run registry:smoke` | 公開後の`irisout@0.3.4`を別アプリへ導入して検査する |
...
`irisout@0.3.4`を別ディレクトリへ導入し、型検査とVite buildを確認します。公開前の
確認には`bun run pack:smoke`を使います。
作者以外による手動試用は未実施です。
