...
> [!WARNING]
> npm公開版は`irisout` 0.2.2です。0.3.0は公開準備済みです。試用版のため、対応範囲を確認してください。
...
| `bun run registry:smoke` | npm公開版`irisout@0.3.0`を別アプリへ導入して検査する |
...
`irisout@0.3.0`を別ディレクトリへ導入し、型検査とVite buildを確認します。
作者以外による手動試用は未実施です。
