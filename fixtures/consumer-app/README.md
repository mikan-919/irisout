...
このディレクトリは、`irisout`を別のVite+アプリへ追加する検証用です。リポジトリ内では
...
bun --cwd fixtures/consumer-app run build
bun --cwd fixtures/consumer-app run dev
```
...
ローカル梱包物を導入し、`bun run registry:smoke`はnpm公開版`irisout@0.2.2`を導入する。
