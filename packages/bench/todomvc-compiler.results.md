...
`filter` 48.7msであった。20%短縮の条件は満たしていない。後続のADR-0046で固定目標を
改訂し、実測した短縮と交換条件を採用してR3を完了した。`focus()`を含む従来値との比較は初期化対象が異なるため、完了判定へ
...
短縮の根拠があるため採用する。後続のADR-0046で固定目標を改訂し、R3を完了した。
