...
`
    expect(() => compile(source)).toThrow(/must reference a function declared after render/)
  })
...
`
    expect(() => compile(source)).toThrow(/only function declarations are allowed after render/)
  })
