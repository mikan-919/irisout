...
describe('M6: cross-milestone no-wrapper verification (ADR-0006)', () => {
  ...
  it('imports one runtime entry containing only the helpers required by used features', () => {
    ...
    expect(imports).toEqual([
      "import { mountWithRanges as __mount__, hydrateWithRanges as __hydrate__, normalizeUseActionResult as __normalizeUseActionResult__, createListRuntime as __createListRuntime__, reconcileList as __reconcileList__, updateListBinding as __updateListBinding__ } from 'irisout/runtime';",
    ])
