...
export type StaticAttr = { name: string; value: string } | { name: string; valueless: true }
...
export function renderStaticAttrs(attrs: StaticAttr[]): string {
  return attrs
    .map((attr) =>
      'valueless' in attr ? ` ${attr.name}` : ` ${attr.name}="${escapeAttrValue(attr.value)}"`,
    )
    .join('')
}

// ADR-0012 決定2: 動的属性バインディングの attribute/property 使い分けの
// 固定表。実需の最小限のみ — 広げるときは ADR-0012 に追記する。
// render.ts(焼き込み)と codegen.ts(更新行)の両方から使う。
export type AttrBindingKind = 'boolProp' | 'prop' | 'attr'

export function attrBindingKind(name: string): AttrBindingKind {
  if (name === 'checked' || name === 'disabled') return 'boolProp'
