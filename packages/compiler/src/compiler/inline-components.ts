...
import type { HandlerFns } from './render.ts'
import { renderCallJsx, splitComponentZones } from './render.ts'
...
function copyPositionsDeep(clone: unknown, original: unknown): void {
  ...
  if (Array.isArray(clone) && Array.isArray(original)) {
    for (let i = 0; i < clone.length; i++) copyPositionsDeep(clone[i], original[i])
    return
  }
...
function substituteProps(
  ...
  for (const attrPath of jsxPath.get('openingElement').get('attributes')) {
    ...
    if (valueNode == null) {
      ...
      throw new Error(`compile: unsupported prop value form on "${tagName}" (scope limit)`)
    }
  }
...
// zone宣言の挿入位置になる)。
function findRenderStmt(componentPath: NodePath<t.FunctionDeclaration>): NodePath<t.Statement> {
...
function expandComponentRef(
  ...
  // 属していないと正しく引けない)。
  const [clonedFnPath] = targetPath.insertAfter(cloneWithPositions(targetPath.node)) as [
