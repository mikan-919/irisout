...
| ブラウザでの初期化          | `onMount()`内で辞書URL取得、Suzume生成、Worker生成を行い、cleanupで破棄する                 | build-time executionは初期HTMLだけを作り、ブラウザ専用APIを実行しない。処理のWorker分離は第5段階で行う |
...
Workerで本文解析を行う処理、連続入力、古い非同期結果の破棄は第5段階へ残す。
