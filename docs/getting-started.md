---
title: npm公開版から始める
description: irisout 0.3.3を空のディレクトリへ導入し、JSXの型検査と本番ビルドを確認する手順
...
この手順は、空のディレクトリへ`irisout` 0.3.3を導入し、JSXの型検査、開発サーバー、
...
cd irisout-app
npm init -y
npm install irisout@0.3.3 vite-plus@0.3.0
...
0.3.0で確認済みの組み合わせは`vite-plus` 0.3.0とTypeScript 5.9である。
...
接続する処理である。要求ごとのHTML生成は`irisout/ssr`と`irisout/hono`から利用できる。
標準の`irisout/vite`だけを使う場合はビルド時の初期HTMLになるため、JavaScript実行前から
リストや条件分岐の内容が必要な用途ではSSR入口を使う。
...
生成部品自体は`unmount()`を持つが、0.3.0の`irisout/vite`は仮想モジュールを読み込むと
