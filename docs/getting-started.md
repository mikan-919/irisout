...
この手順は、空のディレクトリへ`irisout` 0.2.1を導入し、JSXの型検査、開発サーバー、
...
cd irisout-app
npm init -y
npm install irisout@0.2.1 vite-plus@0.3.0
...
0.2.1で確認済みの組み合わせは`vite-plus` 0.3.0とTypeScript 5.9である。
...
接続する処理である。irisout 0.2.1は要求ごとに全画面のHTMLを生成するサーバー描画を
...
生成部品自体は`unmount()`を持つが、0.2.1の`irisout/vite`は仮想モジュールを読み込むと
