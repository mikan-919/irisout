...
**決定済み・実装済み、ADR-0046で効果を採用**(2026-09-09)
...
- List itemのmarker探索は、小さなtemplateの要素木を深さ優先で走査する。selector parserを
  itemごとに起動せず、root自身のmarkerも先に確認する。
...
対して現行69.9msで、短縮率は15.5%である。20%短縮の条件には届かなかったが、ADR-0046で
固定目標を改訂し、この効果を採用してR3を完了した。従来のfocusを含む326.6msとの差は入力区間が
異なるため、効果の根拠に使わない。
