...
- module scopeの`derived`/`collection`、永続化、request単位SSR分離、非同期scheduler、
  汎用store registryは対象外とする。
...
- **module共有derived/collectionも同時に受理する**: 再計算・keyed item状態の所有者を
  追加で定義する必要があり、signalの同期購読という最小縦切りから逸脱するため拒否する。
...
式に限る。複雑なstore、module共有derived/collection、request単位の分離は別契約である。
