...
予算係数は 4.5x とする。2026-07-18にhydration marker検証の固定費で3xから
4xへ変更し、2026-09-04にADR-0018のcomponent instance factoryという意図した
固定費で実測4.38xになったため、直近の余白だけを持つ4.5xへ変更した。

#### Scenario: サイズ予算内
- **WHEN** `examples/counter.jsx` をビルドした `dist/app.js` のサイズが
  手書き基準ファイルのサイズの4.5倍以内である
- **THEN** サイズ予算テストは成功し、実測比がテスト出力に表示される

#### Scenario: サイズ予算超過
- **WHEN** `dist/app.js` のサイズが手書き基準ファイルのサイズの4.5倍を
  超える
