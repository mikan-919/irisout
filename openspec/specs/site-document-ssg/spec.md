...
生成物は文書一覧と文書詳細を静的HTMLとして出力し、JavaScriptを無効にしたブラウザから直接URL、本文、目次、前後リンクを閲覧できなければならない(SHALL)。生成物にはtitle、description、正規URL、OGPを含めなければならない(SHALL)。
...
- **WHEN** 文書ページを生成する
- **THEN** title、description、正規URL、OGP、目次、前後リンクが静的HTMLへ含まれる

### Requirement: 文書入力の検査

生成は重複slug、未知の分類、分類内の順序重複、相対・絶対の文書または見出し参照切れ、切れた画像、登録されていない例、許可されないHTMLまたはコード言語、path traversal、不正なpercent encodingを検出した場合に失敗しなければならない(SHALL)。`/docs`は文書一覧への絶対参照として許可する(SHALL)。
