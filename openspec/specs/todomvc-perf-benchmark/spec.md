...
### Requirement: 初期化性能変更の採否

初期化性能を変える実装は、同一Chromium、同一入力、同一反復数、同一初期化区間で
変更前後を比較し、原因分析と再現方法を記録しなければならない(SHALL)。固定した短縮率
だけで採否を決めず、100件・1,000件・10,000件の初期化、通常更新、転送量、
JavaScriptヒープ、既存機能の回帰を併せて確認しなければならない(SHALL)。Reactとの
初期化方式や計測区間が異なる場合、その値は参考値として区別しなければならない(SHALL)。

#### Scenario: 目標値に届かない改善の採否

- **WHEN** 変更が事前の短縮率目標に届かないが、測定のばらつきを超える短縮と原因が
  同一条件の比較で確認される
- **THEN** イベント契約、対応範囲、転送量、ヒープ、通常更新との交換条件を記録し、
  追加変更または目標改訂を判断できる

#### Scenario: React比較の区別

- **WHEN** irisoutとReactで初期HTMLの扱いまたは初期化方式が異なる
- **THEN** Reactの時間をirisoutの短縮率や完了判定へ直接使用せず、比較条件とともに
  参考値として記録する
