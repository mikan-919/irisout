...
- [x] 1.1 `src/compiler.ts`の`findRootComponent`が持つ
      「Program直下のFunctionDeclarationを名前で集める」ロジックを
      共有ヘルパーへ切り出す(インライン化パスと`findRootComponent`の
      両方から使う)。
- [x] 1.2 ユニットテスト: 複数コンポーネントが宣言されたソースから
      正しい名前→NodePath表が得られること。

## 2. インライン化前処理パスの骨格

- [x] 2.1 新規モジュール(例: `src/compiler/inline-components.ts`)に
      ...
      列挙する(まだ置換はしない)。
- [x] 2.2 展開中のコンポーネント名を保持するvisited集合を用意し、
      再訪問時に`compile: recursive component reference "X" is not
      supported yet (scope limit)`を投げる(自己再帰・相互再帰の両方を
      テストでカバー)。
- [x] 2.3 コンポーネント参照JSX要素が子要素を持つ場合、
      `compile: component children are not supported yet (scope limit)`
      を投げる。
- [x] 2.4 `src/compiler.ts`の`compile()`から`inlineComponents(ast)`を
      `assertTopLevelShape`の直後・`findRootComponent`の前に呼び出す。

## 3. props置換

- [x] 3.1 呼び出し先関数の`ObjectPattern`仮引数からプロパティ名→
      パラメータ名の対応表を作る(shorthand形のみ対応、それ以外の分割
      代入形は`scope limit`)。
- [x] 3.2 呼び出し箇所JSXの各`JSXAttribute`から対応する実引数式を取り出し、
      クローンした呼び出し先本体を`path.scope.getBinding`で解決した
      パラメータ参照ごとに実引数式のクローンで置換する。
- [x] 3.3 テスト: プロパティ参照の置換、シャドーイングされた同名ローカル
      変数が誤って置換されないこと。
- [x] 3.4 (レビューで発覚したハードニング) props参照が呼び出し先本体の
      式全体でも同名bare identifierでもない、より大きな式にネストした
      位置(三項演算子の条件部・メンバー式・二項式等)で使われる場合、
      render()のEditリスト方式では呼び出し先の古いソーステキストが
      そのまま出力に残ってしまう(黙って壊れたコードを生成する、design.md
      D3参照)。`assertSafeToSubstitute`で検出し`scope limit`で拒否する。
      値なしのboolean-shorthand属性(`<Foo enabled/>`、対応する実引数
      テキストがソース上に存在しない)も同じ理由で拒否する。テスト:
      別名propのネスト参照・boolean-shorthandそれぞれの拒否、同名
      bare identifierのネスト参照(`todo.completed`パターン)は引き続き
      許可されることの回帰確認。

## 4. ルートスコープでの単純なインライン化(props置換のみ、ローカルsignalなし)

- [x] 4.1 呼び出し箇所がコンポーネント本体のトップレベル(構造ユニット外)
      にある場合の展開: 呼び出し先の`render()`引数JSX(props置換済み)を
      呼び出し箇所へ`replaceWith`する。
- [x] 4.2 呼び出し先の変数ゾーンsignal/derived宣言を、呼び出し元の変数
      ゾーンへ(呼び出し箇所を含んでいた文の直前に)挿入する。
- [x] 4.3 呼び出し先の動きゾーンfunction宣言を、呼び出し元の動きゾーンへ
      統合する。
- [x] 4.4 名前衝突検出とコンポーネント名リネーム(design D4): signal出力
      変数名・動きゾーン関数名それぞれについて、統合前に呼び出し元の
      既存識別子と衝突する場合のみ`${componentName}_${name}`へリネームし、
      本体内の参照も追随させる。衝突しない場合は素の名前のまま。
- [x] 4.5 テスト: 子要素を持たないシンプルなコンポーネント(例:
      `Footer({ count })`)がトップレベルで参照され、`render.ts:444-448`の
      scope limitに引っかからずコンパイルが通り、生成コードが期待どおり
      動くこと。
- [x] 4.6 テスト: 2つの異なるコンポーネントが同名のsignalを宣言し、
...
- [x] 5.3 インライン化パス(セクション2/3)が、構造ユニット内の呼び出し
...
- [x] 5.6b `body.localHandlers`のうち`writeDeclIds`が`ctx.localDeclIds`と
      ...
      既存の`update()`(item仮引数がある場合はそれを渡す ― 完全な
      bare呼び出しにすると`function update(__next__) { itemParam =
      __next__; ... }`がitemParamをundefinedで潰してしまうため)への
      呼び出しを追記する配線を追加する(同一ユニット直下限定なので
      関数宣言の巻き上げにより参照は曖昧にならない)。
...
- [x] 6.1 `TodoApp`から`TodoItem`コンポーネントを切り出す: `todo`・
      ...
      依存に収まる。
- [x] 6.2 冒頭コメントを実態に合わせて更新する: UNRESOLVED-04は
      ADR-0014により解消・実装済みにする一方、フィクスチャ全体は
      (sole-child制約・3階層ネスト・UNRESOLVED-07により)引き続き
      コンパイルできないことを明記する。
- [x] 6.3 `examples/todomvc.handwritten.js`との整合を確認した ―
      **更新不要と判断**。同ファイルの`createTodoItem`は既に`let editing`
      をfactory内のアイテム専有ローカル変数として持っており(146行目)、
      ADR-0014の「ローカルsignal」概念と構造的に一致済み。span/input
      入れ替え(07)がstyle-display方式である点も、同ファイル自身の
      既存コメント(157-162行目)が「生成コードの目標出力とはこの点で
      一致しない」と明示済みの、本change以前からの既知の乖離であり、
      本changeとは無関係。`examples/todomvc.jsx`自体は無関係な
      pre-existingのscope limitで引き続きコンパイルできないため、
      照合すべき新しいコンパイラ出力も無い。
- [x] 6.4 同一ファイル内コンポーネント合成+ローカルsignalの動作を検証する
